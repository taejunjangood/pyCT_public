...
ext1 = Extension('projectionGPU',
                 sources=["pyCT/forward/projectionGPU.pyx", "pyCT/forward/forward.cu"],
                 language="c++",
                 runtime_library_dirs=[CUDA["lib64"]] if not IS_WINDOWS else None,
                 library_dirs=[CUDA["lib64"]],
                 libraries=["cudart"],
                 include_dirs=[NUMPY_INCLUDE, CUDA["include"], "pyCT/forward"]
                )
ext2 = Extension('reconstructionGPU',
                 sources=["pyCT/backward/reconstructionGPU.pyx", "pyCT/backward/backward.cu"],
                 language="c++",
                 runtime_library_dirs=[CUDA["lib64"]] if not IS_WINDOWS else None,
                 library_dirs=[CUDA["lib64"]],
                 libraries=["cudart"],
                 include_dirs=[NUMPY_INCLUDE, CUDA["include"], "pyCT/backward"]
                )
